...
std::optional<Endpoint> parse_endpoint(std::string_view text)
{
  ...
  while (true) {
    ...
    if (octet == 4 || part.empty() || part.size() > 3) {
      return std::nullopt;
    }
    unsigned value = 0;
...
std::expected<UdpSocket, TransportError> UdpSocket::open(
...
std::expected<std::size_t, TransportError> UdpSocket::send_to(
  std::span<const std::byte> data, const Endpoint & to) const
{
  ...
  const ssize_t n =
    ::sendto(fd_, data.data(), data.size(), 0, reinterpret_cast<const sockaddr *>(&sa), sizeof(sa));
  if (n < 0) {
    return std::unexpected(from_errno(TransportErrorCode::kSendFailed, errno));
  }
...
std::expected<Datagram, TransportError> UdpSocket::recv_one(std::span<std::byte> buffer) const
{
  ...
  if (*r == 0) {
    return std::unexpected(TransportError{TransportErrorCode::kWouldBlock, EAGAIN});
  }
  return d;
}
