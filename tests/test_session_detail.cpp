...
std::vector<std::byte> ack_frame(
  std::uint32_t seq, std::uint16_t cmd_id, std::span<const std::byte> data)
{
...
TEST_CASE("match_ack classifies datagrams", "[session][detail]")
{
  ...
  SECTION("other seq / cmd_id / source ip are late")
  {
    ...
    CHECK(
      detail::match_ack(ack, Endpoint{{192, 168, 1, 99}, kCommandPort}, 1, 0x0000, kLidarIp)
  ...
  {
    CHECK(
      detail::match_ack(GOLDEN(param_config_req), kFrom, 2, 0x0100, kLidarIp).error() ==
      detail::AckMismatch::kNotAck);
    CHECK(
      detail::match_ack(GOLDEN(info_push), kFrom, 7, 0x0102, kLidarIp).error() ==
      detail::AckMismatch::kNotAck);
  }
  SECTION("corrupted frame is bad")
  {
    ...
    CHECK(
      detail::match_ack(bad, kFrom, 1, 0x0000, kLidarIp).error() == detail::AckMismatch::kBadFrame);
    CHECK(
      detail::match_ack({}, kFrom, 1, 0x0000, kLidarIp).error() == detail::AckMismatch::kBadFrame);
  }
}
...
TEST_CASE("parse_discovered_device", "[session][detail]")
{
  ...
  SECTION("serial number without NUL terminator is truncated to 16 chars")
  {
    ...
    for (char c : std::string_view("0123456789ABCDEF")) {
...
TEST_CASE("typed ACK conversions", "[session][detail]")
{
  ...
  SECTION("simple ACK")
  {
    ...
    CHECK(
      detail::to_simple_ack(raw_ack(0x0200, bytes({0x01}))).error().kind ==
      SessionErrorKind::kLidarRejected);
    CHECK(
      detail::to_simple_ack(raw_ack(0x0200, {})).error().kind == SessionErrorKind::kBadResponse);
  }
}
