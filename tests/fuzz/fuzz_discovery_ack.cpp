...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
  ...
  const std::span<const std::byte> datagram{reinterpret_cast<const std::byte*>(data + kPrefix),
                                            size - kPrefix};
  const auto dev = detail::parse_discovered_device(datagram, from);
  if (!dev) return 0;
  const auto frame = parse_command_frame(datagram);
  fuzz::require(frame.has_value() && frame->header.cmd_id == 0x0000 &&
                frame->header.cmd_type == CmdType::kAck);
  fuzz::require(dev->from == from);
  fuzz::require(dev->serial_number.size() <= 16);
  fuzz::require(std::find(dev->serial_number.begin(), dev->serial_number.end(), '\0') ==
