...
def main() -> None:
    ...
    parts.append(
        cpp_bytes(
            "param_config_ack_ok", p.CommandFrame(2, 0x0100, 1, 1, bytes([0, 0, 0])).encode()
        )
    )
    parts.append(
        cpp_bytes(
    ...
    # control commands
    parts.append(
        cpp_bytes("reboot_req", p.CommandFrame(5, 0x0200, 0, 0, p.encode_reboot(100)).encode())
    )
    parts.append(
        cpp_bytes("factory_reset_req", p.CommandFrame(6, 0x0201, 0, 0, b"\0" * 16).encode())
    )
    parts.append(
        cpp_bytes(
            "gps_time_req",
